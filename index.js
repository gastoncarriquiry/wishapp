...
mongoose.connect(`mongodb+srv://${userDB}:${passDB}@cluster0.cwrzz.mongodb.net/wishapp?retryWrites=true&w=majority`);
...
app.post("/login", (req, res) => {
    Usuario.findOne({ mail: { $eq: req.body.mail }, pass: { $eq: req.body.pass } }, (err, usuario) => {
        if (err) {
...
app.post("/postDeseo", (req, res) => {
    deseo.findOne({
        ...
        if (err) {
            res.json({ mensaje: "Error al ingresar el artículo, inténtelo de nuevo más tarde.", codigo: 404 });
...
app.delete("/deseos/:idEliminar", (req, res) => {
    deseo.findByIdAndDelete(req.params.idEliminar, (err, deseoEliminado) => {
        if (err) {
            res.json({mensaje: "Error al eliminar el artículo, inténtelo de nuevo más tarde.", codigo: 404});
        } else {
...
app.put("/deseos/:idModificar", (req, res) => {
    deseo.findByIdAndUpdate(req.params.idModificar, { titulo: req.body.titulo, precio: req.body.precio, tienda: req.body.tienda, link: req.body.link}, (err, deseoModificado) => {
        if (err) {
            res.json({mensaje: "Error al modificar el artículo, inténtelo de nuevo más tarde.", codigo: 404});
        } else {
